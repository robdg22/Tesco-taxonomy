...
type NavigationLevel = "superDepartmentGrid" | "departmentTabs" | "shelfGrid" | "productListing"
...
export default function TaxonomyPrototype3Page() {
  ...
  const [fetchedBranches, setFetchedBranches] = useState<Map<string, TaxonomyItem[]>>(new Map()) // Cache for children
    ...
    async (categoryId: string | null, style: "rounded" | "thumbnail"): Promise<TaxonomyItem[] | null> => {
      ...
      try {
        ...
        if (result.data?.taxonomy) {
          // If categoryId is null, we're fetching root (super departments).
          // Otherwise, we're fetching children of a specific category.
